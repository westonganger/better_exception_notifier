...
class SnsNotifierTest < ActiveSupport::TestCase
  ...
  test 'should initialize aws notifier with received params' do
    ...
    BetterExceptionNotifier::SnsNotifier.new(@options)
  end
  ...
  test 'should send a sns notification in background' do
    ...
    sns_notifier = BetterExceptionNotifier::SnsNotifier.new(@options)
  ...
  test 'should put data from env["better_exception_notifier.exception_data"] into text' do
    controller = mock('controller')
    controller.stubs(:action_name).returns('index')
    controller.stubs(:controller_name).returns('examples')
    ...
    sns_notifier.call(@exception,
                      env: {
                        ...
                        'REQUEST_URI' => '/examples',
                        'action_controller.instance' => controller,
                        'better_exception_notifier.exception_data' => {current_user: 12}
                      })
  end
