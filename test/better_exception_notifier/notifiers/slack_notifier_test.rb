...
class SlackNotifierTest < ActiveSupport::TestCase
  def setup
    ...
    BetterExceptionNotifier::SlackNotifier.any_instance.stubs(:clean_backtrace).returns(fake_cleaned_backtrace)
  ...
  test 'should send a slack notification if properly configured' do
    ...
    slack_notifier = BetterExceptionNotifier::SlackNotifier.new(options)
  ...
  test 'should pass along environment data' do
    ...
    notification_options = {
      env: {
        'better_exception_notifier.exception_data' => { foo: 'bar', john: 'doe' }
      },
      data: {
        'user_id' => 5,
