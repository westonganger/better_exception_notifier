...
class WebhookNotifierTest < ActiveSupport::TestCase
  test 'should send webhook notification if properly configured' do
    BetterExceptionNotifier::WebhookNotifier.stubs(:new).returns(Object.new)
    webhook = BetterExceptionNotifier::WebhookNotifier.new(url: 'http://localhost:8000')
    ...
    assert_includes response[:body][:exception][:backtrace], '/better_exception_notifier/test/webhook_notifier_test.rb:48'
  ...
  test 'should send webhook notification with correct params data' do
    ...
    webhook = BetterExceptionNotifier::WebhookNotifier.new(url: url)
  ...
  def fake_response
    {
      ...
      body: {
        exception: {
          ...
          backtrace: '/better_exception_notifier/test/webhook_notifier_test.rb:48:in `/'
        },
        data: {
