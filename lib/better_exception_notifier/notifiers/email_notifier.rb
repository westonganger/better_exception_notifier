...
module BetterExceptionNotifier
  class EmailNotifier < BaseNotifier
    DEFAULT_OPTIONS = {
      ...
      template_path: 'better_exception_notifier',
      deliver_with: nil
    }.freeze
    ...
    module Mailer
      ...
      def self.extended(base)
        base.class_eval do
          #send(:include, BetterExceptionNotifier::BacktraceCleaner)

          # Append application view path to the BetterExceptionNotifier lookup context.
          append_view_path "#{File.dirname(__FILE__)}/views"

          def better_exception_notifier(env, exception, options = {}, default_options = {})
            load_custom_views

            @env        = env
            @exception  = exception

            env_options = env['better_exception_notifier.options'] || {}
            ...
            @data       = (env['better_exception_notifier.exception_data'] || {}).merge(options[:data] || {})
            @sections += %w[data] unless @data.empty?

            compose_email
          end

          def background_better_exception_notifier(exception, options = {}, default_options = {})
            load_custom_views
          ...
          def compose_subject
            ...
            if @options[:normalize_subject]
              subject = EmailNotifier.normalize_digits(subject)
            end
          ...
          def compose_email
            ...
            name = @env.nil? ? 'background_better_exception_notifier' : 'better_exception_notifier'
            ...
            if @options[:mailer_settings]
              mail.delivery_method.settings.merge!(@options[:mailer_settings])
            end
    ...
    def create_email(exception, options = {})
      ...
      send_notice(exception, options, nil, base_options) do |_, default_opts|
        if env.nil?
          mailer.background_better_exception_notifier(exception, options, default_opts)
        else
          mailer.better_exception_notifier(env, exception, options, default_opts)
        end
      end
    end
